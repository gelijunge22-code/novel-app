...
(async () => {
  const chrome = spawn(CHROME, [
    ...
    '--user-data-dir=/tmp/gh-g4-profile',
  ...
  for (let i = 0; i < 80; i++) {
    await sleep(500);
    try { t = await (await fetch('http://127.0.0.1:' + PORT + '/json/list')).json(); if (t && t.length) break; } catch (e) { if (i % 20 === 19) console.log('  等待中…', e.message); }
  }
  if (!t || !t.length) { console.log('❌ 浏览器没起来'); process.exit(1); }
  console.log('  ✅ 浏览器就绪（第 ' + (t ? 'ok' : '?') + '）');
