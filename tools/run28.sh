#!/bin/bash
# 第 28 轮收口：AI「用得上、看得见」——这一轮的账（看了什么/能用的工具/真调了什么/改了哪些文件）
#              长进气泡下面那张卡片 + 顶栏标题给小方块让位（修 chatsafe 那条真红）
...
say "⓪b 这一轮的账（AI 用得上 · 看得见）：真数据对账 + 真跑一轮 + 写文件记账"
server/venv/bin/python tools/check_chat_usage.py; echo "chatusage exit=$?"
say "⓪c 反证 CHATUSAGE_NODIGEST=1（没记账）"
CHATUSAGE_NODIGEST=1 server/venv/bin/python tools/check_chat_usage.py; echo "chatusage-nodigest exit=$?"
say "⓪d 反证 CHATUSAGE_DROP=entities（现状少一段）"
CHATUSAGE_DROP=entities server/venv/bin/python tools/check_chat_usage.py; echo "chatusage-drop exit=$?"
...
say "①a3 反证 CHATSTREAM_NOWATCH=1（看门狗不在 → 干等）"
CHATSTREAM_NOWATCH=1 server/venv/bin/python tools/check_chatstream.py; echo "chatstream-nowatch exit=$?"
...
say "⑥b 界面：这一轮用了什么那张卡片（真路径出卡片 + 点开有明细 + 跟服务端对账）"
node tools/e2e-chatusage.js; echo "chatusage-ui exit=$?"
say "⑥c 反证 CHATUSAGE_FORCE=hide（把卡片抹掉 = 修之前那版）"
CHATUSAGE_FORCE=hide node tools/e2e-chatusage.js; echo "chatusage-ui-hide exit=$?"
...
CHATSAFE_FORCE=noclip node tools/e2e-chatsafe.js; echo "chatsafe-noclip exit=$?"
