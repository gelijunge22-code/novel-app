...
一个**自己的小说写作台**：自研后端 + 移动端网页，打开浏览器就能用；
还带一个可选的安卓壳（空白通用版，连你自己的服务器）。
...
仓库里的 **`app/写作台-通用版.apk`** 是一个**空白通用版**：

- **里面没有任何书稿**，也不带任何人的服务器地址
- 第一次打开 → 登录页上点「服务器设置」→ 填**你自己那台**的地址
- 然后用自己的登录口令进去就行

**在网页版里也能下载它**：设置 → 关于 → 安卓安装包 → 下载 / 更新。

### 想要一个"地址已经填好"的版本？

```bash
SERVER=http://你的地址:8899/ SEED=0 bash apk/build.sh
```

打完在 `apk/手机写作台.apk`。
`SEED=0` = 不带书稿快照（保持空白）；`SERVER=` = 把地址写进 App 的默认值。
（不传 `SERVER` 就是通用版，第一次打开让用户自己填。）

> App 只是个壳：界面在包里秒开，数据全在你的服务器上。
