...
def _setting(key: str, default: str = "") -> str:
    ...
    第 31 轮就栽在这儿：判据自己写设置时是直接塞裸 JSON，接口那条路塞的是转义过的，
...
def user_sources() -> list[dict]:
    """用户自己加的站点（设置里 `web.sources`，JSON 数组）。**只增不减**：加进来的算白名单尾部。"""
...
def host_of(url: str) -> str:
    ...
    （第 31 轮实测踩到：工具老老实实说"不在名单里"，其实是这一行没去端口）。"""
...
def _parse_mwsearch(body: str, base: str = "", eng_name: str = "萌娘百科") -> list[dict]:
    ...
    这样 web_fetch 才抓得到。**`base` 必须传引擎自己的域名**：第 31 轮用户加了自己的站，
