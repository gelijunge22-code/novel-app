/* ============================================================
   ...
   为什么单独一个文件：
     ...
     预设的切换器在没书时直接隐藏。结果就是用户说的"很多地方指向书架第一本"。
...
(function () {
  ...
  const BookCtx = {
    ...
    /** 「当前书」那一条（全站唯一的组件 · 只有这一份实现）。
        **形态 = 永久长条 + 挂在顶栏里**（第 46 轮用户原话：
        ...
        第 17 / 20 轮修掉的那三个毛病**一条都不许带回来**：
          ...
          · 不许自带底色（那时它"自己要了一条横线"）；
          · 不许假占位（那时盒高 0 却溢出 → 压到别的字上，用户报的"穿模"）。
        参数：
    ...
    bar(slug, onSwitch, opts) {
      ...
      const name = slug
        ...
        : (o.emptyName || '还没有作品');
      /* 第 46 轮：小方块（`.bk-mini`，里面是书名首字 `first`）按用户点名删掉了，
  ...
  /* ── 顶栏右上角那个小方块：全站唯一的一份实现 ──────────────────────
     为什么长在顶栏、而不是正文上面：用户说它「单独一个图层……固定在那儿动不了，
