...
(function () {
  ...
  const BookCtx = {
      ...
      App.sheet('<div class="sheet-grip"></div>' +
        ...
        '<div class="bk-note">' + (o.global
          ? '一键切换：预设 / 设定 / 对话 / 写作台 / 世界 / 剧情 / 质检 / 统计 / 素材 / 术语 / 笔记 / 听书，' +
            '全站一次性换成这本的，跟另一本完全隔开。'
          /* 文案规矩（）：沉稳、中性、专业。原来这句是「切过去之后：这本自己的…」
    ...
    bar(slug, onSwitch, opts) {
      ...
      const name = slug
        ...
        : (o.emptyName || '还没有作品');
      /* ：小方块（`.bk-mini`，里面是书名首字 `first`）按删掉了，
    ...
    mount(el, onSwitch, opts) {
      ...
      const draw = () => {
        ...
        /* ：长条**挂在顶栏里**（用户：「放在最上面……不要给他专门一个独特的区域」）。
        ...
        if (tb) {
          tb.appendChild(bar);
          /* ⚠ 实测到的真 bug（同一屏可能重新 mount 一次 → 堆出两份）：
  ...
  /* ：`installChip()`（往顶栏塞小方块）整段删掉 —— 形态回到旧版那一行之后它没人用了，
