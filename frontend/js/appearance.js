/* ============================================================
   ...
   2. **地址一律走 API.media()**：App 里是 file:// 来源，媒体地址必须换成带口令的绝对地址；
      ...
      带上前导斜杠会解析到站点根，在 /novel/ 这种子路径下就是 404（踩过）。
      ⚠ 别在这儿自己拼 URL。
   3. **图铺在"屏幕自己身上"**（跟的纸纹同一处，见 css/decor.css），
      不是 body 上一张固定层 —— 阅读器（#screen-reader）**保持干净**：
      正文要读，底纹跟字打架最不划算。理由写进 docs/自审清单.md 那一节。
