...
(function () {
  ...
  /* ═══════════════ 顶上的控制块：收进「抽屉」 ═══════════════ */
  /* 此处要求「我用白线隔开了，**白线上面的是完全固定在那里的，完全动不了**，
  ...
  function ensureStrip() {
    ...
    /* 这一行**自己**绑点击 —— 不能挂在别人身上：上一版就是挂错容器 + 被
       对方的 `if (!b) return` 提前 return 掉，表现是"点不了、像有 bug"（此处要求）。 */
  ...
  /* ── 联网搜索那枚胶囊 ────────────────────────────────────────
     此处要求：默认跟主创共用一个模型（后端 web.enabled 只管"能不能上网"），
  ...
  /* ⚠ 第 42 轮抓到的**真 bug**（就是此处说明"刷新之后我发出来的消息看不到了，
     只有一个棕色的像圈一样的东西"）：
