...
(function () {
  ...
  const TOOL_LABEL = {
    ...
    /* 接上来的：界面上本来就有这些能力，AI 之前够不着 —— 名字必须是**中文人话**，
  ...
  function ensureStrip() {
    ...
    run.id = 'chat-run';
    S.runRow = run;
    /* ：**「干活方式」和轮次胶囊摆在同一行**（用户：「展开之后这里非常非常的挤」）。
    ...
    /* 这一行**自己**绑点击 —— 不能挂在别人身上：上一版就是挂错容器 + 被
       对方的 `if (!b) return` 提前 return 掉，表现是"点不了、像有 bug"（）。 */
  ...
  /* ── 联网搜索那枚胶囊 ────────────────────────────────────────
     ：默认跟主创共用一个模型（后端 web.enabled 只管"能不能上网"），
  ...
  /* 切模式要**看得见地**变：说明文字 / 状态条 / 输入框提示 三处一起跟着走。
     （以前只把胶囊点成白色，此处说明"像没用"。判据：这三处必须真的变，见 tools/e2e-chatsafe.js 己） */
  const PLACEHOLDER = {
  ...
  /* 模型名去掉渠道前缀（"[渠道]某模型-flash" → "某模型-flash"），
  ...
  /* ── 自动滚动：**以用户的意图为准**（此处指出"一滑就被拽回去、来回跳"）
  ...
  /* ⚠ 抓到的**真 bug**（就是此处说明"刷新之后我发出来的消息看不到了，
     只有一个棕色的像圈一样的东西"）：
  ...
  /* ── 逐字显现（打字机）────────────────────────────────────────────────
     ...
     实测（2026-09-21，两条渠道都验了）：**渠道把整段正文一次给全** ——
       · "修补站"（某渠道-shim，声明 openai-responses）：`/responses` 流式只回 `data: [DONE]`，
         `/chat/completions` 流式能吐，但**一次就是一大截**（实测 2 片）；
       · 某渠道 官方：`/responses` 只回**一个** `response.output_text.delta`，正文整段塞在里面。
  ...
  /** 一次性拿到**整段**正文时的入口（`report_result` / 快照 / 会话条目都走它）。
      ...
      直接画完就是此处说明"一次性蹦出来"。这里把它塞进同一套逐字显现的队列。 */
  ...
  /* 模型那边出错 / 一个字都没回 —— **必须让用户看见**。
     此处指出"发出去出不了字"就是这里以前什么都没有：错误更新被丢掉、空回复不吭声，
  ...
  /* ── 「发出去一直没字」的看门狗 ────────────────────────────────────────
     。
     把"渠道报错/空回复"都补上了；**还剩最后一种：渠道连得上、就是不吐字**
  ...
  /* 新对话：开一个干净会话。
     此处指出「我没看到有个新对话，相当于不能新建对话」—— 功能一直在（newSession），
  ...
  function handleEvent(data) {
    ...
    if (type === 'message_start') {
      /* ⚠ ：SSE 一接上，服务端会把**这个会话历史事件重放一遍**（这是补课用的）。
