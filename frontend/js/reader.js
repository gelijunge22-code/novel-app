...
(function () {
  ...
  /* ---------- 底部快捷面板（此处要求：别整屏盖住，一边调一边看得到字） ----------
     此处要求、"点开设置之后，UI 界面要统一要好看"。
     ...
     额外两条新规矩：
       ...
       ⑤ **目录从这个面板里搬出去**（见下面 `#rd-toc`）—— 第 39 轮此处要求外观回到旧版，
  ...
  function quickTab(t) {
    ...
    /* 「目录」这一格：**面板让位**，开的是独立的 #rd-toc。
       （第 23 轮此处要求要目录独立、别"像在设置里点开了目录"；第 39 轮此处要求外观回到旧版，
  ...
  function openQuick(tab) {
    /* 面板本身（第 40 轮，此处要求）：**几乎霸占整屏**（92% 屏高）、**一个滚动容器装全部内容**
       （抓手条 + 7 格 + 内容一起滚，没有"上固定下滚动"两层）、7 格是 **3 列等宽网格 + 图标**。
       此处要求 */
    /* 「目录」已经**搬出这个面板**（此处要求："只是相当于在设置里面点开了目录一样，
  ...
  /* 目录里长按一章 = 改名 / 删除。
     以前每行右边挂一个小「更多」按钮（此处要求删掉："右上角的三个点完全去掉……不许两套"），
