...
(function () {
  ...
  const Shelf = {
    ...
    render() {
      ...
      /* 事件**只绑一次**：绑在 #shelf-list 上做委托，render() 换的是里面的卡片、不换它自己。
         这一行是第 20 轮大改重写 render() 时漏掉的 —— 后果是**书卡点了没反应、整本书打不开**
      ...
      /* 顶栏那条「数据是旧的」排在最前：连不上服务器时它是唯一说得清的东西。
         ...
         不然脚本会以为"书架出错时什么都没说"（第 29 轮就是这么误报的）。 */
      if (this.stale) {
  ...
  async function loadTerms(slug) {
    ...
    try {
      ...
      /* 术语表也算一份来源：用户在「术语」面板登记的名字，正文里也该点得出来。
         第 19 轮实测发现：原来只认世界实体（名字多是「原著·某某人物」这种条目名），
  ...
  /* ---------- 底部快捷面板（用户点名：别整屏盖住，一边调一边看得到字） ----------
     用户原话："希望移到下面……稍微弄的小一点，不要那么大，整个屏幕全都弄上去了，
     就弄的小一点，一边调也能看到小说"、"点开设置之后，UI 界面要统一要好看"。
     ...
     额外两条新规矩：
       ...
       ⑤ **目录从这个面板里搬出去**（见下面 `#rd-toc`）—— 第 39 轮用户要求外观回到旧版，
  ...
  /* 第 40 轮：面板改成"几乎霸占整屏 + **一个滚动容器装全部内容**"（抓手条 + 7 格 + 内容一起滚），
  ...
  function ttsActHtml() {
    ...
    return '<button class="rd-act ' + st + '" data-a="tts">' + iconHtml(icon) + '<span>' + label + '</span></button>' +
      ...
      /* 用户报"App 里听书没声音"时的入口：点一下逐项亮灯，把卡在哪一环说清楚 */
  ...
  function quickTab(t) {
    ...
    /* 「目录」这一格：**面板让位**，开的是独立的 #rd-toc。
       （第 23 轮用户点名要目录独立、别"像在设置里点开了目录"；第 39 轮用户要求外观回到旧版，
  ...
  function openQuick(tab) {
    /* 面板本身（第 40 轮，用户点名）：**几乎霸占整屏**（92% 屏高）、**一个滚动容器装全部内容**
       （抓手条 + 7 格 + 内容一起滚，没有"上固定下滚动"两层）、7 格是 **3 列等宽网格 + 图标**。
       用户原话："点开设置会几乎霸占整个屏幕……不会那么拥挤，看不了正文也行。" */
    /* 「目录」已经**搬出这个面板**（用户点名："只是相当于在设置里面点开了目录一样，
  ...
  /* ══════════════ 目录 #rd-toc：**独立的一整块**（第 2 屏） ══════════════
     用户原话：「点开目录之后，只是相当于在设置里面点开了目录一样，并没有那种简洁的感觉，
               而且很乱，最主要的是那个地方还有点挡着的感觉，就是它不是很舒服」。
  ...
  function tocOpen() {
    ...
    /* 有别的面板立着 → 等它**真的收下去**再开目录。
       ...
       （监督人探针实测：面板高度 0）—— 也就是用户说的"点开目录跟没点一样/闪一下"，
    ...
    /* 从底栏「目录」进：设置面板让位。**用 fromBack=true 收它** —— 历史那一格留给目录，
       不然两层面板各压一格、返回键要按两下才退得出去（用户报过"退出到别的工具里"那类怪事）。 */
  ...
  /* 目录里长按一章 = 改名 / 删除。
     以前每行右边挂一个小「更多」按钮（用户点名删掉："右上角的三个点完全去掉……不许两套"），
    ...
    if (!R.audio) {
      ...
      R.audio.addEventListener('ended', () => {
        /* ⚠ 解锁用的那个静音片段也在**同一个** <audio> 上 —— 它播完会立刻触发 ended，
           ...
           （第 33 轮实测到的：解锁后 currentTime 刚走到 0.24 秒就跳到下一段。） */
      ...
      R.audio.addEventListener('error', () => {
        ...
        /* 以前这里只有两句笼统的 toast（"连不上服务器" / "语音加载失败"），
           不带 HTTP 码、不带 audio.error.code、不带地址 —— 用户说"没声音"，
  ...
  async function ttsStart() {
    ...
    if (!meta || !meta.count) {
      ttsStop(false);
      /* 说清楚**到底为什么没声**（用户报"听书没声音"的时候，这一句就是唯一线索）：
  ...
  /* ══════════ 听书自检：把"点了听书没声"从哑巴失败变成看得见 ══════════
     用户报的是「App 里听书从来没有出过声音」，而界面上什么都不说。
  ...
  const Reader = {
    ...
    /* ⚠ 这两个必须**把 fromBack 透传下去**：App.show() 从阅读器换到别的屏时会调
       ...
       屏上还是阅读器（第 30 轮截图判据抓到的：App.show('settings') 之后
